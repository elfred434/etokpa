...
export default function AdminCatalogPage() {
  ...
  const { rows: produits, err, loading, reload } = useLiveRows(() => adminApi.getProducts({ per_page: 100 }));
  ...
  <header className="h-16 flex justify-between items-center px-lg bg-white sticky top-0 z-40 border-b border-border-default"> <div className="flex items-center gap-4"> <span className="font-h2 text-h2 font-bold text-primary">Gestion du catalogue</span> </div> <div className="flex items-center gap-6"> <div className="relative hidden lg:block"> <MIcon name="search" className="absolute left-3 top-1/2 -translate-y-1/2 text-text-tertiary" /> <input className="pl-10 pr-4 py-2 bg-bg-secondary border border-border-default rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all w-64" placeholder="Rechercher un produit..." type="text" value={q} onChange={(e) => setQ(e.target.value)} /> </div> <div className="flex items-center gap-4 border-l border-border-default pl-6">  </div> </div> </header>  <div className="p-lg space-y-lg">  <div className="flex flex-col md:flex-row md:items-center justify-between gap-md"> <div className="flex gap-lg border-b border-border-default w-full md:w-auto"> {([['produits', `Produits (${produitsF.length})`], ['categories', `Catégories (${catsF.length})`], ['packs', `Packs (${packsF.length})`]] as const).map(([k, label]) => (
              ...
              ))} </div> <button className="flex items-center gap-2 bg-primary-container hover:bg-primary-hover text-white px-md py-2.5 rounded-lg font-bold shadow-lg shadow-primary/10 transition-transform active:scale-95" data-onclick="document.getElementById('editModal').classList.remove('hidden')"> <MIcon name="add" />
                    Nouveau produit
                </button> </div>  <div className="bg-white p-md rounded-lg shadow-sm flex flex-wrap items-center gap-4 border border-border-default"> <div className="flex-1 min-w-[200px]"> <div className="relative"> <MIcon name="search" className="absolute left-3 top-1/2 -translate-y-1/2 text-text-tertiary text-sm" /> <input className="w-full pl-9 pr-4 py-2 text-label bg-white border border-border-default rounded-lg focus:ring-1 focus:ring-primary outline-none" placeholder="Nom, SKU ou ID..." type="text" value={q} onChange={(e) => setQ(e.target.value)} /> </div> </div> <select className="px-md py-2 text-label bg-white border border-border-default rounded-lg focus:ring-1 focus:ring-primary outline-none min-w-[140px]" value={catF} onChange={(e) => setCatF(e.target.value)}> <option value="">Catégorie: Tout</option>
                {cats.map((c: any) => (
                  <option key={c.id} value={String(c.id)}>{c.nom}</option>
                ))}
              </select> <select className="px-md py-2 text-label bg-white border border-border-default rounded-lg focus:ring-1 focus:ring-primary outline-none" value={dispoF} onChange={(e) => setDispoF(e.target.value)}> <option value="">Disponibilité</option> <option value="stock">En stock</option> <option value="rupture">Rupture</option> </select> <div className="flex border border-border-default rounded-lg overflow-hidden"> <button className="p-2 bg-bg-secondary text-primary"><MIcon name="format_list_bulleted" /></button> <button className="p-2 hover:bg-bg-secondary text-text-tertiary"><MIcon name="grid_view" /></button> </div> </div>  <div className="bg-white rounded-lg shadow-sm border border-border-default overflow-hidden"> <table className="w-full text-left border-collapse"> <thead> <tr className="bg-bg-secondary border-b border-border-default"> <th className="py-md px-lg w-10"> <input className="rounded border-border-default text-primary focus:ring-primary" type="checkbox" /> </th> <th className="py-md px-md font-label text-label text-text-tertiary uppercase tracking-wider">Produit</th> <th className="py-md px-md font-label text-label text-text-tertiary uppercase tracking-wider">Catégorie</th> <th className="py-md px-md font-label text-label text-text-tertiary uppercase tracking-wider">Prix</th> <th className="py-md px-md font-label text-label text-text-tertiary uppercase tracking-wider">Statut</th> <th className="py-md px-md font-label text-label text-text-tertiary uppercase tracking-wider text-right">Actions</th> </tr> </thead> <tbody>
                {tab === 'produits' && produitsF.map((pr: any) => (
                  <tr key={pr.id} className="hover:bg-bg-secondary/50 transition-colors">
                    ...
                    <td className="py-4 px-md text-right"> <div className="flex gap-2 justify-end"> <button type="button" className="font-semibold text-primary hover:underline" onClick={() => ouvrir(pr)}>Modifier</button> <button type="button" className="font-semibold text-error hover:underline" onClick={() => void supprimer(pr.id)}>Supprimer</button> </div> </td>
                  </tr>
                ))}
                {tab === 'categories' && catsF.map((c: any) => (
                  <tr key={c.id} className="hover:bg-bg-secondary/50 transition-colors">
                    ...
                    <td className="py-4 px-md text-right"> <div className="flex gap-2 justify-end"> <Link to="/admin/categories" className="font-semibold text-primary hover:underline">Éditer</Link> <button type="button" className="font-semibold text-error hover:underline" onClick={() => void supprimerCat(c)}>Supprimer</button> </div> </td>
                  </tr>
                ))}
                {tab === 'packs' && packsF.map((b: any) => (
